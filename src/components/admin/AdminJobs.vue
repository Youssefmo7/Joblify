<template>
    <div class="bg-surface rounded-xl border border-border overflow-hidden shadow-sm">
        ...
        <div v-if="store.loading && store.pendingJobs.length === 0" class="p-12 text-center text-sm text-gray-500 bg-white">
        ...
        <div class="divide-y divide-border bg-white" v-if="store.pendingJobs.length > 0">
            ...
            <div class="px-5 py-3 bg-gray-50/50 flex items-center gap-3">
                <input type="checkbox" :checked="isAllSelected" @change="toggleSelectAll" class="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary" />
                <span class="text-xs font-medium text-gray-500 uppercase tracking-wider">Select all on this page</span>
            </div>

            <div class="p-5 hover:bg-gray-50 transition-colors" v-for="job in store.pendingJobs" :key="job.id">
                <div class="flex items-start gap-3">
                    <input type="checkbox" :value="job.id" v-model="selectedIds" class="mt-1 h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary flex-shrink-0" />
                    <div class="flex-1">
        ...
        <div v-else-if="!store.loading" class="p-12 text-center text-sm text-gray-500 bg-white flex flex-col items-center">
...
export default {
  ...
  setup() {
    ...
    const isAllSelected = computed(() => {
      if (store.pendingJobs.length === 0) return false;
      return store.pendingJobs.every(j => selectedIds.value.includes(j.id));
    });
    ...
    const toggleSelectAll = () => {
      if (isAllSelected.value) {
        ...
        selectedIds.value = store.pendingJobs.map(j => j.id);
      }
    };
