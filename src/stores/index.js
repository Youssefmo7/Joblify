...
const ADMIN_STORAGE_KEY = "joblify_admin_state_v1";

const DEFAULT_MODERATION_QUEUE = [
  {
    id: 301,
    jobId: 1,
    author: "John Miller",
...
function cloneDefaultModerationQueue() {
  const now = Date.now();

  return DEFAULT_MODERATION_QUEUE.map((item, index) => ({
    ...item,
    // Keep predictable but realistic timestamps for time-based filters.
    flaggedAt: new Date(now - (index + 1) * 60 * 60 * 1000).toISOString(),
  }));
}

function buildAdminSnapshot(state) {
  return {
    jobStatuses: state.jobs.map((job) => ({ id: job.id, status: job.status })),
    approvedJobs: state.approvedJobs,
    rejectedJobs: state.rejectedJobs,
...
export const useAppStore = defineStore("app", {
  state: () => ({
    ...
    activityLog: [],
    adminHydrated: false,
    moderationQueue: cloneDefaultModerationQueue(),
  }),

  getters: {
    currentJob(state) {
      return (
        state.jobs.find((j) => j.id === state.currentJobId) || state.jobs[0]
      );
    },
    pendingJobs(state) {
    ...
    approvalRatePercent(state) {
      ...
      return Math.round((state.approvedJobs.length / totalReviewed) * 100);
    },
    actionsLast24h(state) {
    ...
    moderationReasonsCount(state) {
      return state.moderationQueue.reduce((acc, item) => {
        const reason = item.flaggedReason || "Other";
  ...
  actions: {
    ...
    // admin actions
    initAdminState() {
      if (this.adminHydrated) return;

      const snapshot = readAdminSnapshot();
      if (snapshot) {
        if (Array.isArray(snapshot.jobStatuses)) {
          snapshot.jobStatuses.forEach(({ id, status }) => {
            const job = this.jobs.find((j) => j.id === id);
            if (job && typeof status === "string") {
              job.status = status;
            }
          });
        }

        this.approvedJobs = Array.isArray(snapshot.approvedJobs)
          ? snapshot.approvedJobs
          : [];
        this.rejectedJobs = Array.isArray(snapshot.rejectedJobs)
          ? snapshot.rejectedJobs
          : [];
        this.moderationQueue = Array.isArray(snapshot.moderationQueue)
          ? snapshot.moderationQueue.map((item) => ({
              ...item,
              flaggedAt: item.flaggedAt || new Date().toISOString(),
            }))
          : this.moderationQueue;
        this.adminReviewNotes = snapshot.adminReviewNotes || {};
        this.activityLog = Array.isArray(snapshot.activityLog)
          ? snapshot.activityLog
          : [];
      }

      this.adminHydrated = true;
    },
    persistAdminState() {
      ...
      window.localStorage.setItem(
        ADMIN_STORAGE_KEY,
        JSON.stringify(buildAdminSnapshot(this)),
      );
    },
    setJobReviewNote(jobId, note) {
      this.adminReviewNotes[jobId] = note;
      this.persistAdminState();
    },
    addAdminActivity(entry) {
    ...
    approveJob(jobId) {
      ...
      if (job) {
        ...
        this.addAdminActivity({
          ...
          entityId: jobId,
          title: job.title,
          meta: this.adminReviewNotes[jobId] || "No note",
        });
        this.persistAdminState();
      }
    },
    ...
    removeFlaggedComment(commentId) {
      const comment = this.moderationQueue.find((c) => c.id === commentId);
      ...
      if (comment) {
        this.addAdminActivity({
          ...
          entityId: commentId,
          title: `${comment.author} on ${comment.jobTitle}`,
          meta: comment.flaggedReason,
        });
      }
      this.persistAdminState();
    },
    resetAdminModerationState() {
      const reviewedJobIds = Array.from(
        new Set([...this.approvedJobs, ...this.rejectedJobs]),
      );
      reviewedJobIds.forEach((jobId) => {
        const job = this.jobs.find((j) => j.id === jobId);
      ...
      this.activityLog = [];
      this.moderationQueue = cloneDefaultModerationQueue();
      this.persistAdminState();
    },
  },
});
